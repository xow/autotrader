...
class ContinuousAutoTrader:
    ...
    def calculate_technical_indicators(self, market_data: List[Dict[str, Any]], prices: Optional[np.ndarray] = None, volumes: Optional[np.ndarray] = None) -> List[Dict[str, Any]]:
        ...
        if prices is not None and volumes is not None:
            ...
            if len(df) < self.settings.ml.sequence_length: # Use sequence_length from settings
                logger.warning(f"Not enough valid data points to calculate indicators. Need at least {self.settings.ml.sequence_length}, got {len(df)}")
                return market_data
        else:
        ...
        RS = np.where(roll_down1 == 0, 0, roll_up1 / roll_down1)
